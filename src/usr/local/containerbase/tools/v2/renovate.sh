...
function install_tool () {
  ...
  if [[ $(restore_folder_from_cache "${versioned_tool_path}" "${TOOL_NAME}/${TOOL_VERSION}") -ne 0 ]]; then
    ...
    # pin node version
    sed -i --follow-symlinks "1 s:.*:#\!\/opt\/containerbase\/tools\/node\/${node_version}\/bin\/node:" "${versioned_tool_path}/bin/renovate";
    sed -i --follow-symlinks "1 s:.*:#\!\/opt\/containerbase\/tools\/node\/${node_version}\/bin\/node:" "${versioned_tool_path}/bin/renovate-config-validator";
